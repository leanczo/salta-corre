...
function loadInscribed() {
  try {
    const raw = localStorage.getItem("salta-running-inscribed");
...
export default function App() {
  const [inscribed, setInscribed] = useState(loadInscribed);
  ...
  const toggleInscribed = (id) => {
    setInscribed((prev) => {
      const next = { ...prev, [id]: !prev[id] };
      if (!next[id]) delete next[id];
      try { localStorage.setItem("salta-running-inscribed", JSON.stringify(next)); } catch {}
      return next;
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "-apple-system, 'SF Pro Text', 'Segoe UI', system-ui, sans-serif", transition: "background 0.3s, color 0.3s" }}>
      ...
      <div style={{ background: t.headerBg, padding: "24px 16px 20px", textAlign: "center", position: "relative" }}>
        <div className="header-inner" style={{ position: "relative" }}>
          <button onClick={() => setThemeMode(p => p === "dark" ? "light" : "dark")} style={{ position: "absolute", top: -4, right: 0, background: t.accentSoft, border: "none", borderRadius: "50%", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 15 }}>
            {themeMode === "dark" ? "☀️" : "🌙"}
          </button>
      ...
      <div className="content-wrapper">
        ...
        {nextRace && daysUntilNext !== null && (
          <div onClick={() => scrollToCard(nextRace.id)} style={{
            ...
            background: t.countdownBg, border: `1px solid ${t.countdownBorder}`, borderRadius: 12,
        ...
        {/* Filters */}
        <div className="filters-bar" style={{ padding: "13px 12px 5px", overflowX: "auto", display: "flex", gap: 6, scrollbarWidth: "none", WebkitOverflowScrolling: "touch", flexWrap: "wrap" }}>
          {filters.map(f => (
